...
class HTML_admin_misc {
  ...
  function couponsBroadcast()
  {
    $broadcast = new A25_Broadcast(10, 'Coupons',
        'This optional feature lets students enter a coupon code to receive a discount on registration',
         A25_Link::to('administrator/documentation/coupons'));
    echo $broadcast->render();
  }
  
  /**
   * Even when this function isn't used, we should keep it around because we
   * will probably need to use it again in the future.
   */
  function fireBroadcasts()
  {
    foreach (A25_ListenerManager::all() as $listener) {
      if ($listener instanceof A25_ListenerI_HomepageBroadcasts) {
        $listener->homepageBroadcasts();
      }
    }
  }
	...
	*/
	function changelog() {
		...
		<pre>
			<?php
			readfile( ServerConfig::webRoot.'/CHANGELOG.php' );
			?>
		</pre>
		<?php
	}
