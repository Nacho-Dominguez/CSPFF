...
if (isset( $_POST['submit'] )) {
	...
	// Trim entered username to 40 characters, since that is how it
	// is saved:
	if ( strlen($usrname) > 40 ) {
		$usrname = substr( $usrname, 0, 40 );
	}
    ...
    // update db user last visit record corresponding to currently logged in user
		...
		if ( $_VERSION->SITE == 1 && @$mosConfig_admin_expired === '1' ) {
			...
			$checktime = ( $mosConfig_session_life_admin ? $mosConfig_session_life_admin : 10800 ) / 2;
		...
		// if no value set, default is 90 days
			$purge 	= 90;
		}
