...
function frontpage( $gid, $access, $pop, $now ) {
	...
	setThePageTitle($rows[0], $mainframe);
 ...
 */
function setThePageTitle($row, $mainframe)
{
	if (strlen($row->title_alias) < 5 ) {
...
function showCategory( $id, $gid, $access, $sectionid, $limit, $selected, $limitstart, $now, $selected, $filter ) {
	...
	*/
	...
	$pageNav = new mosPageNavFrontend( $total, $limitstart, $limit );
...
function showBlogSection( $id=0, $gid, $access, $pop, $now=NULL ) {
	...
	// Dynamic Page Title
	if ($menu) {
		$mainframe->setPageTitleAlias( $rows[0]->title );
	}
