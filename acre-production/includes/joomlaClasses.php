<?php
/**
 * These classes used to be in /includes/joomla.php.  I moved them for test
 * purposes.  Now joomla.php includes this file.
 */
  ...
  /**
	...
	*/
	function initSession() {
		...
		// Session Cookie `name`
		$sessionCookieName 	= A25_CookieMonster::sessionCookieName();
	...
	*/
	function sessionCookieName() {
		...
		$hash = 'site' . $mainframe->getCfg( 'live_site_nossl' );
    	return md5( $hash );
	...
	*/
	function remCookieName_User() {
		$value = mosHash( 'remembermecookieusername'. A25_CookieMonster::sessionCookieName() );
	...
	*/
	function remCookieName_Pass() {
		$value = mosHash( 'remembermecookiepassword'. A25_CookieMonster::sessionCookieName() );
	...
	*/
	...
	/**
	* @return A25_Record_User A user object with the information from the current session
	*/
	function getUser() {
		global $database;
  ...
  function writableCell( $folder, $relative=1, $text='', $visible=1 ) {
	...
	*/
	function monthSelectList( $tag_name, $tag_attribs, $selected ) {
		$arr = array(
			mosHTML::makeOption( '0', '- Select One -' ),
	...
	*/
	function treeSelectList( &$src_list, $src_id, $tgt_list, $tag_name, $tag_attribs, $key, $text, $selected ) {
		...
		$ilist = mosTreeRecurse( $keys[0], '', array(), $children );
	...
	*/
	...
	function sortIcon( $base_href, $field, $state='none' ) {
		...
		. "<img src=\"ServerConfig::currentUrl()/images/M_images/sort_$state.png\" width=\"12\" height=\"12\" border=\"0\" alt=\"{$alts[$next_state]}\" />"
		. "</a>";
		return $html;
	}
...
/**
 * @deprecated - use A25_Functions::stripSlashesFromStringOrArray(), instead
 */
function mosStripslashes( &$value ) {
    return A25_Functions::stripSlashesFromStringOrArray($value);
}

/**
 * @deprecated - use A25_Functions::bindArrayToObject(), instead
 */
function mosBindArrayToObject( $array, &$obj, $ignore='', $prefix=NULL, $checkSlashes=true )
{
    return A25_Functions::bindArrayToObject($array, $obj, $ignore, $prefix, $checkSlashes);
}
...
* @param string A filter for the names
*/
function mosRedirect( $url, $msg='', $code=301) {
...
class mosSession extends mosDBTable {
	...
	*/
	 ...
	 * @return boolean
	 */
	function purge( $inc=10800, $and='' ) {
		global $mainframe;

		if ($inc == 'core') {
			$past_logged 	= time() - $mainframe->getCfg( 'lifetime' );
			$past_guest 	= time() - 10800;
...
function mosToolTip( $tooltip, $title='', $width='', $image='tooltip.png', $text='', $href='#', $link=1 ) {
	...
	if ( !$text ) {
		$image 	= ServerConfig::currentUrl() . '/includes/js/ThemeOffice/'. $image;
...
function mosWarning($warning, $title='Joomla! Warning') {
	...
	$tip 		.= '<img src="'. ServerConfig::currentUrl() .'/includes/js/ThemeOffice/warning.png" border="0"  alt="warning"/></a>';
...
function mosCreateMail( $from='', $fromname='', $subject, $body ) {
    ...
    // Added by Thomas Albright:
    A25_Functions::checkApprovedEmailDomain($mail->From);
...
function mosMail( $from, $fromname, $recipient, $subject, $body, $mode=0, $cc=NULL, $bcc=NULL, $attachment=NULL, $replyto=NULL, $replytoname=NULL ) {
	...
	// Validate email address syntax (Added by Thomas Albright)
	...
	*/
	function mosTabs( $useCookies, $xhtml=NULL ) {
		...
		if ( $xhtml ) {
			...
			echo "<link id=\"luna-tab-style-sheet\" type=\"text/css\" rel=\"stylesheet\" href=\"" . ServerConfig::currentUrl(). "/includes/js/tabs/tabpane.css\" />";
		}

		echo "<script type=\"text/javascript\" src=\"". ServerConfig::currentUrl() . "/includes/js/tabs/tabpane_mini.js\"></script>";
...
class mosAdminMenus {
	...
	*/
	function ImageCheck( $file, $directory='/images/M_images/', $param=NULL, $param_directory='/images/M_images/', $alt=NULL, $name='image', $type=1, $align='middle', $title=NULL, $admin=NULL ) {
		...
		if ( $param ) {
			$image = A25_Link::to($param_directory . $param);
			if ( $type ) {
			...
			if ( file_exists( $mosConfig_absolute_path . $path . $file ) ) {
				$image = A25_Link::to($path . $file);
			} else {
				// outputs only path to image
				$image = A25_Link::to($directory . $file);
			}
...
class mosCommonHTML {
	 ...
	 *
	 * NOTE: This function does not cooperate with integration tests.  When
	 * putting a function under test, A25_Javascript::loadOverLib()
	 * must be used instead.  This function interferes with other running tests,
	 * so using it will make the test pass by itself, but fail in AllTests.
	 */
	function loadOverlib() {
		...
		// check if this function is already loaded
			?>
			<script language="javascript" type="text/javascript" src="<?php echo ServerConfig::currentUrl();?>/includes/js/overlib_mini.js"></script>
			<script language="javascript" type="text/javascript" src="<?php echo ServerConfig::currentUrl();?>/includes/js/overlib_hideform_mini.js"></script>
	...
	/*
	 * @deprecated - use jQuery UI DatePicker instead, as in the Course Edit form
	 *
	 * @todo - Remove all uses of this function, replacing with jQuery UI
	 * DatePicker.  Then, remove this function, and remove the javascript files
	 * from source control.
	 *
	...
	*/
	function loadCalendar() {
		?>
		<link rel="stylesheet" type="text/css" media="all" href="<?php echo A25_Link::to('/includes/js/calendar/calendar-mos.css');?>" title="green" />
		<!-- import the calendar script -->
		<script type="text/javascript" src="<?php echo A25_Link::to('/includes/js/calendar/calendar_mini.js');?>"></script>
		<!-- import the language module -->
		<script type="text/javascript" src="<?php echo A25_Link::to('/includes/js/calendar/lang/calendar-en.js');?>"></script>
		<?php
	}
 ...
 * @return string
 * Deprecated. Use A25_CookieMonster::hash instead.
 */
function mosHash( $seed ) {
    return A25_CookieMonster::hash($seed);
}
...
/**
 * Returns the top level ancestor id
 *
...
function getTopId( $id ) {
	...
	if ($id == 99999999 || $id == 0 || $id == '') {
		return 0;
	}

	$query = "SELECT id,parent FROM #__menu WHERE id=" . $id;
	$database->setQuery( $query );
	$results = $database->loadObjectList();
	$result = $results[0];

	while($result->parent != 0){
		$query = "SELECT id,parent FROM #__menu WHERE id=" . $result->parent;
		$database->setQuery( $query );
		$results = $database->loadObjectList();
		$result = $results[0];
	}
