...
class database {
	...
	* @param boolean If true and there is an error, go offline (no longer in effect)
	*/
	function database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
		// perform a number of fatality checks, then die gracefully
		if (!function_exists( 'mysql_connect' )) {
			throw new Exception ("mysql_connect() does not exist");
		}
		if (!($this->_resource = @mysql_connect( $host, $user, $pass, true ))) {
			throw new Exception ("mysql_connect() failed.  Is host '" . $host . "' correct?");
}
		if ($db != '' && !mysql_select_db( $db, $this->_resource )) {
			throw new Exception ("mysql_select_db() failed");
			$mosSystemError = 3;
		}
	...
	*/
	...
	/**
	* Load an array of single field results into an indexed array
	*/
