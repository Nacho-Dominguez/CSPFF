...
if (is_dir($abspath_folder)) {
	...
	if (!$the_image) {
		...
		// Modifications for HTI
		$id = $Itemid;
		do {
			$image_name = $id . '.' . $type;
			if (file_exists($abspath_folder . '/' . $image_name)) {
				break;
			} else {
				$id = getParentId($id);
			}
		...
		//functionality changed to NOT SHOW RANDOM IMAGES
		if($id != 0) {
	 	...
	 	<div align="center">
		 	...
		 	<div class="colHeaderPic"><img src="<?php echo $image; ?>" border="0" alt="<?php echo $image_name; ?>" style="width: 100%;"/><br /></div>
		 	<?php
		  	if ($link) {
		  		?>
		  		</a>
	  	...
	  	<?php
		} //end if on NOT SHOWING RANDOM IMAGES
	}
}
?>
